...
- Non-standard Markdown extensions (e.g., GitHub Flavored Markdown features) that are not part of the CommonMark spec.
- Inconsistent use of tabs vs. spaces for indentation (CommonMark treats tabs as 4-space stops for block structure but does not expand them in content).
- Using backslash escapes in code spans or HTML tags, where they are treated as literal characters rather than escapes.
- Relying on bare URLs being auto-linked without angle brackets, as this is not part of the CommonMark specification
- Using `_` for intraword emphasis, which is not allowed in CommonMark (use `*` instead).
